...
class SingerPage extends Component {
  ...
  onDeleteTrack = parameter => {
    const {trackList} = this.state
    const filteredByDelete = trackList.filter(each => each.id !== parameter)
    this.setState({trackList: filteredByDelete})
  }

  filteredBySearchResult = () => {
    const {trackList, userInput} = this.state
    const filteredBySearch = trackList.filter(each =>
      each.name.toLowerCase().includes(userInput.toLowerCase()),
    )
    this.setState({trackList: filteredBySearch})
  }

  onUpdateSearch = event => {
    this.setState(
      {userInput: event.target.value},
      this.filteredBySearchResult(),
    )
  }
  ...
  renderNoSongView = () => {
    ...
    return (
      <div className="singer-page-container">
